...
```mermaid
---
title: Cookie and Web Storage data flow
---
flowchart TD
  Browser["Browser storage context"] --> Cookie["Cookie scoped by domain, path, and attributes"]
  Browser --> Local["localStorage scoped by origin"]
  Browser --> Session["sessionStorage scoped by origin and tab"]
  Cookie --> Request["Eligible cookies accompany HTTP requests"]
  Request --> Server["Server"]
  Local --> Persistent["Persists until cleared or evicted"]
  Session --> Tab["Scoped to one top-level tab session"]
  Script["Same-origin JavaScript"] --> Local
  Script --> Session
  Script -. "unless HttpOnly" .-> Cookie
```
