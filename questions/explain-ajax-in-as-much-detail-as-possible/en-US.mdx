...
3. **Handling the response**: The `Response` object exposes the HTTP status and methods such as `.json()`, `.text()`, and `.blob()` for consuming its body. Check `response.ok` or `response.status` before treating the request as successful.
...
6. **Error handling**: Network, CORS, abort, and body-parsing failures reject a promise and can be handled with `.catch()` or `try...catch`. HTTP error statuses still produce a fulfilled `Response`, so application code must check the status explicitly.
