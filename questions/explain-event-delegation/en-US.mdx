...
```
You might expect that each `<li>` gets its own click event listener. But:

**Without Event Delegation:** The browser would have to manage N event listeners for N items.

**With React's Event Delegation:** Only a single event listener for click is attached at the root (where React renders). The event bubbles up from the `<li>`, and React's synthetic event system figures out which handler to call.
