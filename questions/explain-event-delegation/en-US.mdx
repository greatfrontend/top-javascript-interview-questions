...
## One ancestor listener for many descendants

Delegation relies on bubbling: a stable ancestor receives descendant events and decides which matching child originated the interaction.

```mermaid
---
title: Event delegation through a common ancestor
---
flowchart TD
  Click["User clicks a descendant"] --> Target["Event target is the deepest clicked node"]
  Target --> Bubble["Event bubbles to the common ancestor"]
  Bubble --> Listener["Ancestor listener calls target.closest(selector)"]
  Listener --> Match{"Matching descendant inside the ancestor?"}
  Match -->|"Yes"| Handle["Handle the delegated action"]
  Match -->|"No"| Ignore["Ignore the event"]
  Dynamic["Descendant added later"] --> Click
```

Because the listener belongs to the ancestor, newly added matching descendants work without separate listener registration.
...
1. **Determine the target**: Within the event listener, you can inspect the event object to identify the actual target of the event (the child element that triggered the event). Use `event.target` to find which specific child element was interacted with. (`event.currentTarget` refers to the ancestor the listener is attached to, not the child that triggered the event.)
...
1. **Fewer listeners**: Event delegation reduces listener bookkeeping and can reduce memory use when dealing with a very large number of elements.
1. **Dynamic elements**: It works with dynamically added or removed child elements, as the common ancestor continues to listen for events on them.
...
- **Listener count is often not the bottleneck.** Modern browsers handle many listeners efficiently, so measure the actual page rather than relying on a fixed listener-count rule.
...
For many event types, [React](https://react.dev/) installs native listeners on the React root and dispatches synthetic events through the component tree. Some event types are handled differently because they do not delegate normally, so this is an implementation detail rather than a guarantee that every React handler maps to one root listener.
...
This lets React provide a consistent event API and centralize much of its event dispatch. Application code should still choose delegation based on the DOM behavior it needs rather than assuming it is always faster.
