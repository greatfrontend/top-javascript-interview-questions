...
`bind()` returns a wrapper that stores a target function, a bound receiver, and any leading arguments.

```mermaid
---
title: Bound function invocation
---
flowchart LR
  Target["Original function"] --> Bind["bind(boundThis, arg1)"]
  Receiver["Bound this value"] --> Bind
  Preset["Preset arguments"] --> Bind
  Bind --> Bound["New bound function"]
  Later["Later call arguments"] --> Bound
  Bound --> Invoke["Invoke target with bound receiver and concatenated arguments"]
```

Calling `bind()` does not run the target. If the bound function is later used with `new`, constructor semantics ignore the stored receiver but retain preset arguments.
