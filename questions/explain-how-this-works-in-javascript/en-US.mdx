...
## Resolve `this` from the function and call form

For ordinary functions, `this` is usually selected at invocation time; arrow functions are the major lexical exception.

```mermaid
---
title: JavaScript this resolution
---
flowchart TD
  Call["A function is invoked"] --> Arrow{"Arrow function?"}
  Arrow -->|"Yes"| Lexical["Use this from the enclosing lexical scope"]
  Arrow -->|"No"| New{"Called with new?"}
  New -->|"Yes"| Instance["Use the new instance"]
  New -->|"No"| Explicit{"Bound or called with call/apply?"}
  Explicit -->|"Yes"| Supplied["Use the supplied this value"]
  Explicit -->|"No"| Method{"Called as object.method()?"}
  Method -->|"Yes"| Receiver["Use the object before the dot"]
  Method -->|"No"| Default["undefined in strict mode; global object in sloppy non-module code"]
```

Copying a method into a variable changes its call form and can therefore change `this`.

## `this` keyword

In JavaScript, `this` is a keyword that refers to the current execution context of a function or script. It's a fundamental concept in JavaScript, and understanding how `this` works is crucial for building maintainable applications.

### Used globally

At the top level of a classic browser script, `this` refers to `window`. At the top level of an ES module, it is `undefined`. Node.js also depends on the module system: top-level `this` in CommonJS is `module.exports`, not `globalThis`, while it is `undefined` in an ES module.
...
// call/apply/bind cannot replace an arrow function's lexically captured `this`.
obj.arrowFunction.call(anotherObj); // Uses the surrounding scope's `this`.
obj.arrowFunction.apply(anotherObj); // Uses the surrounding scope's `this`.
const boundArrowFunction = obj.arrowFunction.bind(anotherObj);
boundArrowFunction(); // Still uses the surrounding scope's `this`.
```

### Within event handlers

When a regular function is called as a DOM event listener, `this` refers to the element on which the listener was registered (`event.currentTarget`). It is not necessarily the element where the event originated (`event.target`). In this example both values are the same button.
...
document.getElementById('my-button').addEventListener('click', () => {
  console.log(this); // Lexically captured `this`, not the button element.
});
```

In summary, `this` in JavaScript refers to the current execution context of a function or script, and its value can change depending on the context in which it is used. Understanding how `this` works is essential for building maintainable JavaScript applications.
