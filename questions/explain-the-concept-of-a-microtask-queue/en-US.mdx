...
```mermaid
---
title: Microtask checkpoint between browser tasks
---
flowchart LR
  Task["Run one task"] --> Sync["Finish its synchronous stack"]
  Sync --> Check{"Microtask queued?"}
  Check -->|"Yes"| Microtask["Run one microtask"]
  Microtask --> Enqueue["It may enqueue more microtasks"]
  Enqueue --> Check
  Check -->|"No"| Render["Browser may render"]
  Render --> Next["Select the next task"]
  Next --> Task
```
