...
```mermaid
---
title: Cache lookup decision flow
---
flowchart TD
  Request["Request data or an asset"] --> Lookup["Look up cache entry"]
  Lookup --> Found{"Entry found?"}
  Found -->|"No"| Origin["Fetch or compute from origin"]
  Found -->|"Yes"| Fresh{"Entry still fresh?"}
  Fresh -->|"Yes"| Hit["Return cached value"]
  Fresh -->|"No"| Revalidate["Revalidate or refresh"]
  Revalidate -->|"Not modified"| Hit
  Revalidate -->|"New value"| Origin
  Origin --> Store["Store with expiry or validation metadata"]
