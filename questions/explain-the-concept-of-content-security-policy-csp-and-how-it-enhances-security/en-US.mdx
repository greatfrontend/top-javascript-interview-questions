...
---
flowchart TD
  Server["Server response"] --> Header["Content-Security-Policy header"]
  Header --> Browser["Browser installs policy for the document"]
  Browser --> Attempt["Script, style, frame, image, or connection attempt"]
  Attempt --> Match{"Allowed by the applicable directive?"}
  Match -->|"Yes"| Allow["Load or execute"]
  Match -->|"No"| Block["Block the attempt"]
  Block --> Report["Optionally send a violation report"]
```
