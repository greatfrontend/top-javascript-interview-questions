...
```mermaid
---
title: Lexical environment lookup
---
flowchart TD
  Use["Inner function reads name"] --> Inner{"Binding in inner scope?"}
  Inner -->|"Yes"| InnerValue["Use inner binding"]
  Inner -->|"No"| Outer{"Binding in enclosing function?"}
  Outer -->|"Yes"| OuterValue["Use enclosing binding"]
  Outer -->|"No"| Global{"Binding in global scope?"}
  Global -->|"Yes"| GlobalValue["Use global binding"]
  Global -->|"No"| Error["Throw ReferenceError"]
```
