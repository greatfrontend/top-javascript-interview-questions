...
A WebSocket is stateful: messages can flow only after the opening handshake succeeds and before either peer finishes closing the connection.

```mermaid
---
title: WebSocket ready-state lifecycle
---
stateDiagram-v2
  [*] --> Connecting: new WebSocket(url)
  Connecting --> Open: opening handshake succeeds
  Connecting --> Closed: handshake fails
  Open --> Open: send and receive messages
  Open --> Closing: close initiated
  Closing --> Closed: closing handshake or timeout
  Open --> Closed: transport failure
  Closed --> [*]
```

A closed WebSocket instance cannot reconnect; retry logic creates a new instance and should usually apply backoff.

## What is the WebSocket API?

The WebSocket API establishes a persistent, full-duplex communication channel between a client (usually a web browser) and a server. Unlike HTTP's request-response message model, either WebSocket peer can send a message after the connection is established. HTTP can reuse or multiplex network connections, so the distinction is the communication model—not necessarily one new transport connection per HTTP request.
...
Here is a basic non-live example. The endpoint is a placeholder because an executable example should not depend on an external service or leave a persistent connection open.
