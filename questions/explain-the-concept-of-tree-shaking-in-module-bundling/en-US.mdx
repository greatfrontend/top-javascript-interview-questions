...
```mermaid
---
title: Tree-shaking reachability analysis
---
flowchart TD
  Modules["Static ES module graph"] --> Roots["Mark entry points and used exports"]
  Roots --> Trace["Trace reachable imports and declarations"]
  Trace --> Used["Keep reachable code"]
  Trace --> Unused["Identify unreachable exports"]
  Unused --> Effects{"Can removal preserve side effects?"}
  Effects -->|"Yes"| Drop["Remove unused code"]
  Effects -->|"No or unknown"| Keep["Keep potentially effectful code"]
  Used --> Bundle["Generate optimized bundle"]
  Drop --> Bundle
  Keep --> Bundle
```
