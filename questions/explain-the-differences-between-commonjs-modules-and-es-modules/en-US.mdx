...
```mermaid
---
title: CommonJS and ES module loading models
---
flowchart TD
  Source["Module source"] --> System{"Module system"}
  System -->|"CommonJS"| Require["Execute require() at runtime"]
  Require --> LoadCJS["Load and evaluate the requested module"]
  LoadCJS --> Exports["Return module.exports"]
  System -->|"ES modules"| Parse["Parse static import and export declarations"]
  Parse --> Graph["Build and link the module graph"]
  Graph --> Evaluate["Evaluate modules in dependency order"]
  Evaluate --> Bindings["Expose live imported bindings"]
```
