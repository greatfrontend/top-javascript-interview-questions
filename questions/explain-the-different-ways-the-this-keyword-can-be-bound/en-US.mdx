...
---
flowchart TD
  Function["Resolve this"] --> Arrow{"Arrow function?"}
  Arrow -->|"Yes"| Lexical["Lexical this"]
  Arrow -->|"No"| Constructor{"new invocation?"}
  Constructor -->|"Yes"| NewBinding["New instance binding"]
  Constructor -->|"No"| Bound{"Bound function or call/apply?"}
  Bound -->|"Yes"| Explicit["Explicit binding"]
  Bound -->|"No"| Method{"Method invocation?"}
  Method -->|"Yes"| Implicit["Receiver binding"]
  Method -->|"No"| Default["Default binding"]
```
...
const greet = makeGreeter.call({ firstName: 'Frank' });
greet.call({ firstName: 'Grace' }); // 'Frank'; call() cannot rebind the arrow
```
