...
```mermaid
---
title: SEO-friendly single-page application delivery
---
flowchart TD
  URL["Request a route URL"] --> Render["Server render or serve pre-rendered route HTML"]
  Render --> HTML["Return meaningful content, title, metadata, and links"]
  HTML --> Consumer{"Consumer"}
  Consumer -->|"Crawler"| Index["Discover and index route content"]
  Consumer -->|"Browser"| Hydrate["Hydrate interactive application"]
  Hydrate --> Navigate["Client-side navigation"]
  Navigate --> Update["Update URL, content, title, and metadata per route"]
```
