...
```mermaid
---
title: Authentication and authorization request flow
---
flowchart TD
  Request["HTTPS request"] --> Credential["Validate session or access token"]
  Credential --> Valid{"Credential valid?"}
  Valid -->|"No"| Unauthenticated["Reject or require authentication"]
  Valid -->|"Yes"| Principal["Establish trusted principal"]
  Principal --> Policy["Evaluate action, resource, ownership, and context"]
  Policy --> Allowed{"Explicitly authorized?"}
  Allowed -->|"No"| Forbidden["Deny by default"]
  Allowed -->|"Yes"| Handler["Perform the requested operation"]
  Handler --> Audit["Record security-relevant outcome"]
```
