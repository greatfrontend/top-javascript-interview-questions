...
```mermaid
---
title: Concatenated and parameterized SQL queries
---
flowchart TD
  Input["Untrusted input"] --> Build{"Query construction"}
  Build -->|"String concatenation"| SQLText["Input becomes part of SQL text"]
  SQLText --> Alter["Attacker can alter query structure"]
  Alter --> Database["Database executes unintended command"]
  Build -->|"Parameterized query"| Template["Fixed SQL template with placeholders"]
  Input --> Bound["Bound parameter value"]
  Template --> Driver["Database driver or prepared statement"]
  Bound --> Driver
  Driver --> Safe["Database treats the value as data"]
```
