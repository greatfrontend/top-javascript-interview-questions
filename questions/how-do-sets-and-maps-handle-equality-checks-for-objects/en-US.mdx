...
In JavaScript, `Set`s and `Map`s use object identity to determine whether two object values are the same. Two separately created objects remain distinct even when they contain equal-looking properties.
...
- Internally, `Set`s and `Map`s compare values using the [SameValueZero](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Equality_comparisons_and_sameness#same-value-zero_equality) algorithm. For objects this reduces to reference equality, while for primitives it behaves like `===` except that `NaN` is considered equal to itself.
