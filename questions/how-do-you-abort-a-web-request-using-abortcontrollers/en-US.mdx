...
```mermaid
---
title: AbortController signal fan-out
---
flowchart LR
  Controller["AbortController"] --> Signal["controller.signal"]
  Signal --> FetchA["fetch request A"]
  Signal --> FetchB["fetch request B"]
  Signal --> Listener["abort event listener"]
  Caller["Caller invokes controller.abort(reason)"] --> Controller
  Controller --> Aborted["Signal becomes permanently aborted"]
  Aborted --> FetchA
  Aborted --> FetchB
  Aborted --> Listener
```
