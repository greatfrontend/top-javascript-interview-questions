...
```mermaid
---
title: Asynchronous error channels
---
flowchart TD
  Failure["Asynchronous operation fails"] --> Contract{"Completion contract"}
  Contract -->|"Promise"| Rejection["Reject the promise"]
  Rejection --> Await["Catch around await"]
  Rejection --> Catch["Attach a catch rejection handler"]
  Contract -->|"Error-first callback"| ErrorArg["Invoke callback with an error"]
  ErrorArg --> Branch["Check the error argument before using the result"]
  Rejection --> Missing["No rejection handler"]
  Missing --> Unhandled["Unhandled rejection reporting"]
```
