...
> [!NOTE] Two behaviors are commonly misremembered: `Promise.all([])` does not reject or hang. Its returned Promise is already fulfilled with an empty array, but an attached `.then()` callback still runs asynchronously as a microtask. The result array always preserves input order, even when `promise[2]` resolves before `promise[0]`.
