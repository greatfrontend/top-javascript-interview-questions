...
## Choosing an aggregation outcome

The methods start from the same set of inputs but differ in when and how the aggregate promise settles.

```mermaid
---
title: Promise.all compared with Promise.allSettled
---
flowchart TD
  Inputs["Receive an iterable of values and promises"] --> Method{"Aggregation method"}
  Method -->|"Promise.all"| All{"Does any input reject?"}
  All -->|"Yes"| FailFast["Reject as soon as that rejection is observed"]
  All -->|"No"| OrderedValues["Fulfill with ordered values"]
  Method -->|"Promise.allSettled"| Settle["Wait for every input to settle"]
  Settle --> Records["Fulfill with ordered status records"]
```

Use `all()` when every result is required for success; use `allSettled()` when every outcome must be inspected independently.
...
- **Required independent reads**: loading the current user, permissions, and feature configuration before rendering a page that needs all three.
...
Do not treat `Promise.all()` as a transaction. If several bookings or writes start together, one rejection does not cancel or roll back the others. Use a real transactional boundary where available, or design idempotency and compensating actions explicitly.

## Failure-mode reference

| Input scenario | `Promise.all()` | `Promise.allSettled()` |
| --- | --- | --- |
| Empty iterable `[]` | Returns an already-fulfilled promise containing `[]` | Returns an already-fulfilled promise containing `[]` |
...
> [!NOTE] Empty input and result ordering
>
> `Promise.all([])` does not reject or hang. Its returned Promise is already fulfilled with an empty array, but an attached `.then()` callback still runs asynchronously as a microtask. The result array always preserves input order, even when `promise[2]` resolves before `promise[0]`.
