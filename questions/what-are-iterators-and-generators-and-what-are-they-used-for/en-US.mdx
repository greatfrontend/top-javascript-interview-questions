...
## Generator-backed iteration lifecycle

A generator implements the iterator protocol by pausing at each `yield` and resuming only when its consumer asks for another result.

```mermaid
---
title: Generator and iterator lifecycle
---
stateDiagram-v2
  [*] --> SuspendedStart
  SuspendedStart --> Executing: next()
  Executing --> SuspendedYield: yield value
  SuspendedYield --> Executing: next(input)
  Executing --> Completed: return or reach the end
  SuspendedStart --> Completed: return()
  SuspendedYield --> Completed: return()
  Completed --> Completed: next() returns done true
```

Every `next()` call returns an iterator result with `value` and `done`; once completed, the iterator cannot resume normal execution.
...
async function* fetchDataInBatches(
  url,
  { numBatches = 5, batchSize = 10, signal } = {},
) {
  let startIndex = 0;
  let currBatch = 0;
  ...
  while (currBatch < numBatches) {
    ...
    endpoint.searchParams.set('start', startIndex);
    endpoint.searchParams.set('limit', batchSize);
...
async function fetchAndLogData() {
  ...
  const dataGenerator = fetchDataInBatches('/api/todos', {
...
```js
async function* fetchDataAsyncIterator(url, pagesToFetch = 3, { signal } = {}) {
  let currPage = 1;

  while (currPage <= pagesToFetch) {
    const endpoint = new URL(url, location.origin);
    endpoint.searchParams.set('page', currPage);
    const response = await fetch(endpoint, { signal });
    if (!response.ok) throw new Error(`Request failed (${response.status})`);
    const data = await response.json();
    if (data.length === 0) break;
...
async function fetchAndLogData() {
  const asyncIterator = fetchDataAsyncIterator('/api/todos');
