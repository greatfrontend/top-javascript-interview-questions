...
`EventSource` maintains a long-lived HTTP connection through which the server sends framed events to the browser.

```mermaid
---
title: Server-Sent Events connection and reconnection
---
sequenceDiagram
  participant App
  ...
  EventSource->>Server: HTTP GET with text/event-stream
  loop While connected
    Server-->>EventSource: id, event, and data fields
    EventSource-->>App: message or named event
  end
  Server--xEventSource: Connection closes
  EventSource->>Server: Reconnect, optionally with Last-Event-ID
```

SSE is server-to-client only; client-to-server messages still use another HTTP request or a bidirectional protocol such as WebSocket.
...
- **`Last-Event-ID`**: The client sends the `Last-Event-ID` header when reconnecting, allowing the server to identify the last received event. The server must implement storage and replay of missed events.
...
## Modern uses of SSE: streaming responses and beyond

SSE is a common format for streaming generated text and other incremental server responses. The fit is straightforward: chunks flow from server to client over ordinary HTTP. APIs differ, however: some use SSE, while others use newline-delimited JSON or another streaming format, so clients must follow the protocol documented by the service.

OpenAI is one concrete example: the Responses API emits server-sent events when `stream: true`, including typed incremental events such as `response.output_text.delta`. See the [OpenAI streaming events reference](https://platform.openai.com/docs/api-reference/responses-streaming).
