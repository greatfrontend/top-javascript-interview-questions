...
- **Bookmarking and back-button issues**: the History API (`pushState` and the `popstate` event) and modern routers (Next.js, React Router, Vue Router) handle URL state automatically. AJAX-era apps that broke the back button were generally not using these.
