...
```mermaid
---
title: Reachability in Map and WeakMap
---
flowchart LR
  Root["Reachable application root"] --> Map["Map"]
  Map -->|"strong reference"| MapKey["Object used as a Map key"]
  Root --> WeakMap["WeakMap"]
  WeakMap -. "weak association" .-> WeakKey["Object used as a WeakMap key"]
  WeakKey --> Other{"Another strong reference to WeakKey?"}
  Other -->|"Yes"| Live["WeakKey remains reachable"]
  Other -->|"No"| Collect["WeakKey and its weak entry may be collected"]
```
