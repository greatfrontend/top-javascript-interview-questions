...
## Separate agents communicating by messages

A worker runs in a separate JavaScript agent with its own event loop and global scope; it does not share the page's call stack or DOM.

```mermaid
---
title: Main thread and worker communication
---
sequenceDiagram
  participant Main as Main thread
  participant Clone as Structured clone or transfer
  participant Worker as Worker thread
  Main->>Clone: postMessage(input, transfer?)
  Clone->>Worker: Deliver message event
  Worker->>Worker: Perform CPU-intensive work
  Worker->>Clone: postMessage(result, transfer?)
  Clone->>Main: Deliver message event
  Main->>Main: Update the DOM
```

Messages are copied with the structured-clone algorithm unless transferable ownership is moved, so communication cost remains part of the design.
...
- Can be terminated explicitly, and the browser may stop a worker when its owning documents go away.
...
- **Origin and loading rules**: Worker script loading is subject to worker-type-specific same-origin and CORS rules. For example, classic dedicated worker entry scripts generally must be same-origin, while imported resources and module workers have their own Fetch/CORS behavior.
