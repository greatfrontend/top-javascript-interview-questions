...
```mermaid
---
title: Async function suspension at await
---
sequenceDiagram
  participant Caller
  participant AsyncFn as Async function
  participant Awaited as Awaited promise
  participant Queue as Microtask queue
  Caller->>AsyncFn: Call function
  AsyncFn->>Awaited: Evaluate await expression
  AsyncFn-->>Caller: Return outer promise and suspend
  Awaited->>Queue: Queue continuation when settled
  Queue->>AsyncFn: Resume with value or throw reason
  AsyncFn-->>Caller: Settle outer promise
```
