...
```mermaid
---
title: innerHTML and textContent processing
---
flowchart TD
  Input["String value"] --> API{"Assignment target"}
  API -->|"textContent"| Text["Create or replace text content"]
  Text --> Literal["Markup characters remain visible text"]
  API -->|"innerHTML"| Parse["Parse the string as HTML"]
  Parse --> Nodes["Create DOM nodes from markup"]
  Nodes --> Risk["Untrusted active markup can create XSS"]
```
