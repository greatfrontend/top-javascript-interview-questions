...
```mermaid
---
title: mouseenter and mouseover across nested elements
---
flowchart LR
  Outside["Pointer outside parent"] --> Parent["Pointer enters parent"]
  Parent --> ParentEvents["parent gets mouseenter and mouseover"]
  ParentEvents --> Child["Pointer moves into child"]
  Child --> Over["child gets mouseover; it bubbles to parent"]
  Child --> Enter["child gets mouseenter"]
  Enter --> NoRepeat["parent does not get another mouseenter"]
```
