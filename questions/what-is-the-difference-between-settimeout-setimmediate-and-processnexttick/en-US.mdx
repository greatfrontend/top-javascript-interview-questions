...
```mermaid
---
title: Node.js scheduling checkpoints and phases
---
flowchart TD
  Callback["Current JavaScript operation completes"] --> NextTick["Drain process.nextTick queue"]
  NextTick --> PromiseJobs["Drain promise microtasks"]
  PromiseJobs --> Loop["Continue the event loop"]
  Loop --> Timers["Timers phase: eligible setTimeout callbacks"]
  Timers --> Poll["Poll phase: I/O callbacks and waiting"]
