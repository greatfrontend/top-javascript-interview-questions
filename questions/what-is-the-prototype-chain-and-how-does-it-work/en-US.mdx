...
The prototype chain is easiest to understand as a repeated lookup rather than as copied properties.

```mermaid
---
title: Property lookup through the prototype chain
---
flowchart TD
  Read["Read object.key"] --> Own{"Does object have own key?"}
  Own -->|"Yes"| Value["Return the property value"]
  Own -->|"No"| Prototype["Get object's [[Prototype]]"]
  Prototype --> End{"Is the prototype null?"}
  End -->|"Yes"| Missing["Return undefined"]
  End -->|"No"| Repeat["Use the prototype as the current object"]
  Repeat --> Own
```

Assignment follows additional rules, but ordinary reading never copies an inherited property onto the receiver.
