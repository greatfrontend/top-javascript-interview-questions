...
```mermaid
---
title: Promise.all aggregate behavior
---
flowchart TD
  Inputs["Iterable of values and promises"] --> Normalize["Convert each input with Promise.resolve"]
  Normalize --> Wait["Wait for all inputs"]
  Wait --> Outcome{"What happens first?"}
  Outcome -->|"Every input fulfills"| Values["Fulfill with values in input order"]
  Outcome -->|"Any input rejects"| Reject["Reject with that reason"]
```
