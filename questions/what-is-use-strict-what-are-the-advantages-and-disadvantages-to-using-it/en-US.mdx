...
`"use strict"` is a directive introduced in ECMAScript 5 (ES5) that signals to the JavaScript engine that the code it surrounds should be executed in "strict mode". Strict mode imposes stricter parsing and error handling rules, making your code more secure and less error-prone.
...
- **Build output** may be strict when a tool preserves or emits ES modules or inserts a directive, but this depends on the configured module format. A JavaScript target such as ES2015 does not by itself guarantee strict mode.
...
1. **Compatibility**: Strict mode is Baseline and supported across all modern browsers.
